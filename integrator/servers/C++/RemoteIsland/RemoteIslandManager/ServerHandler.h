...
public:
	...
	ServerHandler(Mutex& mutex, const std::string& );
	~ServerHandler();
	...
	static std::list<SocketThread*> runningThreads;
	int GetAddListCount()
	{
		return (int)this->m_add.size();
	}

	std::map<std::string,std::string> GetPairList(const std::string& path1, const std::string& path2 );
