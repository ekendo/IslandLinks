...
//#ifdef IPPROTO_IPV6
		...
		HTTPServerSocket * hss = dynamic_cast<HTTPServerSocket *>(tmp);
				
		if(hss)
		{
			DEB(printf("evidently this is a HTTPServerSocket\n");)

			HTTPServerSocket* ss = dynamic_cast<HTTPServerSocket *>(tmp);
				
			ss->m_doNotChangeList = this->serverDoNotChangeList;
			ss->m_ignoreList = this->serverIgnoreList;
			ss->m_trackedUrlMap = this->serverTrackerUrlMap;
		}
...
public:
	...
	std::string serverDoNotChangeList;
	std::string serverIgnoreList;
	...
	std::map<std::string,std::string> serverTrackerUrlMap;
};
