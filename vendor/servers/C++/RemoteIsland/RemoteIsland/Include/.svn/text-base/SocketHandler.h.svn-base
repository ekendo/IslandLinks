...
public:
	...
	void Add(Socket *);
	int GetDetachListCount();
	int GetConnectingCount();
	bool SpinNewInstance();
	void SetProxyHost(std::string value);
	void SetForwardMax(int value)
	{
		this->m_portForwardMax = value;
	}
...
public:
	...
	int SelectBalanced(long sec, long usec, int forwardNum);
	...
	void EnableThreadPool(int minThreads);

	SocketThreadPool* GetThreadPool();

	std::list<SocketThread*> GetThreadList();

	void AddToThreadList(SocketThread* st)
	{
		this->v_m_threadList.push_back(st);
	}
...
private:
	...
	int m_portForwardCounter;
	int m_portForwardValue;
	int m_portForwardMax;
	...
	bool m_b_spinNewTracker;
	bool m_b_rampDownMode;
	bool m_useThreadPool;
	bool m_b_forwardPort;
	SocketThreadPool* m_pThreadPool;
	static std::list<SocketThread*> v_m_threadList;
