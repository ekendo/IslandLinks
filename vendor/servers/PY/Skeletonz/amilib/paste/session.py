...
class SessionMiddleware(object):
    ...
    def __init__(self, application, global_conf=None, **factory_kw):
        ...
        self.SessFact = dict({'SessionFactory': None}) 
        print '[session]session middle initd<br>'

    def __call__(self, environ, start_response):
        
        #print '<br>[session]before factory session call<br>'
        session_factory = SessionFactory(environ, **self.factory_kw)
        #print '<br>[session]_call_:after SessionFactory<br>'
        
        #why does this dictionary not accept objects?
        environ['paste.session.factory'] = 'SessionFactory'
        #lets make it a lookup instead then
        self.SessFact['SessionFactory'] = session_factory
        exec_info = self.SessFact
        print '<br>[session]_call_:after SessionFactory dictionary switch<br>'
        ...
        def session_start_response(status, headers, exc_info=None):
            ...
            print '<br>[session]session_start_response called<br>'
            return start_response(status, headers, exc_info)
	
	# this is throwing the 500 error but it calls dispatcher
        app_iter = self.application(environ, session_start_response)
        #app_iter = None
        print '<br>[session]_call_ after app_iter<br>'

	#start_response
        #print '<br>[session]before defs and wsgi lib<br>'

        #might want to rework start
        def start():
            print "[session]_call_:trying to start the session<br>"
	...
	#might want to redo this	
        def close():
            print "[session]_call_:trying to close the session<br>"
            if session_factory.used:
                session_factory.close()
                
        #we probably don;t need this for the cgi app
