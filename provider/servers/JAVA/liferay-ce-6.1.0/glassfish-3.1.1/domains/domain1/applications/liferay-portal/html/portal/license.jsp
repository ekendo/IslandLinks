...
<%@ page import="com.liferay.portal.license.LicenseManager" %>
...
<c:choose>
	<c:when test="<%= clusterNodes.size() <= 1 %>">
		...
		Map<String, String> serverInfo = LicenseManager.getServerInfo();

		List<Map<String, String>> licenseProperties = LicenseManager.getLicenseProperties();
		%>
			...
			<td style="border: 1px solid gray;">
				...
				%>
					...
					<tr>
						...
						<td>
							<%= curLicenseProperties.get("owner") %>
						</td>
						<td>
							<%= curLicenseProperties.get("description") %>
						</td>
						<td>
				...
				sendClusterRequest(
					...
					function(message) {
						...
						for (var i in message) {
							...
							addColumn(row, message[i].owner);
							addColumn(row, message[i].description);
