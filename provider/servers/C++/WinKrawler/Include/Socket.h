...
public:
	...
	virtual void SetForwardPort(int port);
	virtual int GetForwardPort();
	...
	virtual void OnRead();
	/** Called when there is something to be read from the file descriptor. */
	virtual void OnRead(bool portForward,  int newport);
	...
	virtual void OnPortProxyAccept(std::string forwardAddress, std::string forwardParam, int forwardPort);
...
*/
	...
	*/
	...
	int	writeBuf;
	bool debugValue;
	std::string trackerUrls;
	std::string hostRedirect;
