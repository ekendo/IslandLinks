...
public:
	...
	*out = 0;
	...
	void SetSpecificLineMatch(std::string specificLineMatchStart, std::string specificLineMatchEnd)
	{
		this->m_thisSpecificValueStart = specificLineMatchStart;
		this->m_thisSpecificValueEnd = specificLineMatchEnd;
	}

	std::string GetSpecificLineMatchValue()
	{
		return this->m_thisSpecificValue;
	}
	...
	void OnLine(const std::string& line)
	{
		...
		if (line.find(this->m_thisSpecificValueStart)!=std::string::npos)
		{
			std::string::size_type end  = line.find(this->m_thisSpecificValueEnd);
			std::string::size_type start = line.find(this->m_thisSpecificValueStart)+this->m_thisSpecificValueStart.length();

			if(end!=std::string::npos)
			{	
				this->m_thisSpecificValue = line.substr(start,end - start);
			}
		}
	}
...
private:
		...
		std::string m_thisSpecificValueStart;
		std::string m_thisSpecificValueEnd;
		std::string m_thisSpecificValue;
